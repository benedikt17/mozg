...
export function canvasSummaryEntries(
  ...
  const comparePosition = (
    first: CanvasSummarySourceNode,
    second: CanvasSummarySourceNode,
  ) =>
  ...
  const sources = document.edges
    ...
    .flatMap((edge) => {
      ...
      return isCanvasSummarySourceNode(node)
        ? [{ node, order: edge.summaryOrder! }]
        : [];
    })
  ...
  for (const edge of document.edges) {
    ...
    if (!previous || comparePosition(parent, previous) < 0)
      parents.set(edge.targetNodeId, parent);
  }
