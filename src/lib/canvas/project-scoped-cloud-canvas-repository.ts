...
function projectError(
  ...
  if (cause instanceof TypeError || cause instanceof DOMException) {
    return new CloudCanvasRepositoryError(
      "network",
      "Cloud Canvas project request failed.",
      { operation, code },
    );
  }
  return new CloudCanvasRepositoryError(
  ...
  implements CloudCanvasRepository, CanvasGroupRepository
{
  ...
  async listCanvases(workspaceId: string): Promise<CloudCanvasSummary[]> {
    ...
    try {
      const [allowed, canvases] = await Promise.all([
        this.canvasIds(),
        this.repository.listCanvases(this.workspaceId),
      ]);
  ...
  async renameCanvas(
    ...
    try {
      await this.assertCanvas(canvasId);
      return await this.repository.renameCanvas(this.workspaceId, canvasId, title);
    } catch (cause) {
      throw projectError(cause, "rename");
    }
  }
  ...
  async saveCanvasDocument(
    ...
    try {
      ...
      return await this.repository.saveCanvasDocument(input);
    } catch (cause) {
      throw projectError(cause, "save");
    }
  }
  ...
  async loadCanvasViewState(
    ...
    try {
      await this.assertCanvas(canvasId);
      return await this.repository.loadCanvasViewState(this.workspaceId, canvasId);
    } catch (cause) {
      throw projectError(cause, "load-view-state");
    }
  }
  ...
  async saveCanvasViewState(input: SaveCanvasViewStateInput): Promise<void> {
    ...
    try {
      ...
      throw projectError(cause, "save-view-state");
    }
  }
  ...
  async listCanvasGroups(workspaceId: string): Promise<CanvasGroup[]> {
    this.assertWorkspace(workspaceId);
    try {
      const [allowed, groups] = await Promise.all([
        this.groupIds(),
        this.repository.listCanvasGroups(this.workspaceId),
      ]);
  ...
  async renameCanvasGroup(input: RenameCanvasGroupInput): Promise<CanvasGroup> {
    ...
    try {
      ...
      return await this.repository.renameCanvasGroup(input);
    } catch (cause) {
      throw projectError(cause, "rename-group");
    }
  }
  ...
  async softDeleteCanvasGroup(
    ...
    try {
      await this.assertGroup(input.groupId);
      return await this.repository.softDeleteCanvasGroup(input);
    } catch (cause) {
      throw projectError(cause, "delete-group");
    }
  }
  ...
  async moveCanvasGroup(input: MoveCanvasGroupInput): Promise<CanvasGroup> {
    ...
    try {
      await this.assertGroup(input.groupId);
      if (input.parentGroupId) await this.assertGroup(input.parentGroupId);
      return await this.repository.moveCanvasGroup(input);
    } catch (cause) {
      throw projectError(cause, "move-group");
    }
  }
  ...
  async moveCanvasToGroup(input: MoveCanvasToGroupInput): Promise<void> {
    ...
    try {
      ...
      throw projectError(cause, "move-canvas");
    }
  }
