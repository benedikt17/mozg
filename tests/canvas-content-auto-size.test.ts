...
describe("Canvas text and shape content sizing", () => {
  ...
  it("keeps the accepted editor alignment, measurement, wrapping, and save wiring", () => {
    ...
    expect(shellStyles).toMatch(
      /\.textNodeContent\s*\{[^}]*padding:\s*12px;[^}]*overflow-wrap:\s*anywhere;[^}]*word-break:\s*break-word;/su,
    );
    expect(shellStyles).toMatch(
      /\.shapeNodeContent\s*\{[^}]*padding:\s*12px;[^}]*overflow-wrap:\s*anywhere;[^}]*word-break:\s*break-word;/su,
    );
  });
});
